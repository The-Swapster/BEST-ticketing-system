...
def create_table():
    c.execute('create table if not exists bus(bus_number text primary key, route_number text, source text, destination text, seats real, standing_capacity real, driver_name text, conductor_name text)')
    c.execute('create table if not exists seats(bus_number text, women_reserved real, handicaped_reserved real, senior_reserved real,others real, foreign key(bus_number) REFERENCES bus(bus_number))')
    c.execute('create table if not exists bus_stop(name text , route_number text, constraint id primary key (name, route_number), foreign key(route_number) references bus(route_number))')
    c.execute('create table if not exists passenger(bus_number text, route_number text, ticket_id real primary key, start_stop text, end_stop text, type text, foreign key(bus_number) references bus(bus_number), foreign key(route_number) references bus(route_number))')
    c.execute('create table if not exists bus_status(bus_number text, current_location text, passenger_count real, available_seats real, foreign key(bus_number) references bus(bus_number))')
    c.execute('create table if not exists passenger_count(bus_number text, standing real, sitting real, foreign key(bus_number) references bus(bus_number))')

def data_entries_bus():
    c.execute("insert into bus values('MH01CD1234', '1SPL', 'Mumbai CSMT', 'NCPA', 51, 20, 'Vinayak Rao', 'Gaitonde Ganpat')")
    c.execute("insert into bus values('MH01XY6754', '1SPL', 'Mumbai CSMT', 'NCPA', 51, 20, 'Mukund Narvekar', 'Suhas Deshmukh')")
    c.execute("insert into bus values('MH01MD3245', '1SPL', 'Mumbai CSMT', 'NCPA', 51, 20, 'Malhar Holkar', 'Mahesh  Manjrekar')")
    c.execute("insert into bus values('MH01AB3848', '1SPL', 'Mumbai CSMT', 'NCPA', 51, 20, 'Makrand Deshpande', 'Keshav Tilak')")
    c.execute("insert into bus values('MH01BA5705', '1SPL', 'Mumbai CSMT', 'NCPA', 51, 20, 'Upender Shukla', 'Gopal Gokhle')")
    c.execute("insert into bus values('MH01RM5018', 'C6Exp', 'Colaba', 'Dr. Ambedkar Garden', 51, 20, 'Suresh Prabhu', 'Dilip Kumar')")
    c.execute("insert into bus values('MH01UA6550', 'C6Exp', 'Colaba', 'Dr. Ambedkar Garden', 51, 20, 'Mangesh Kale', 'Manohar Parekar')")
    c.execute("insert into bus values('MH01SA7745', 'C6Exp', 'Colaba', 'Dr. Ambedkar Garden', 51, 20, 'Lakshmikant Berede', 'Umesh Wagmare')")
    c.execute("insert into bus values('MH01ZM8199', 'C6Exp', 'Colaba', 'Dr. Ambedkar Garden', 51, 20, 'Ramakant Yadav', 'Girish Kadam')")
    c.execute("insert into bus values('MH01DG1860', 'C6Exp', 'Colaba', 'Dr. Ambedkar Garden', 51, 20, 'Subodh Deshpande', 'Ashok Shroff')")
